...
public class NetflixService extends Service {
    ...
    public void onCreate() {
        ...
        boolean needUpdate = !buildDate.equals(getSavedBuildDate(mContext));

        setNrdpCapabilitesIfNeed(NRDP_PLATFORM_CAP, needUpdate);
        if (needUpdate) {
            setSavedBuildDate(mContext, buildDate);
        }
    }
    ...
    private static String getSavedBuildDate(Context c) {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(c);
        return sp.getString(SAVED_BUILD_DATE, "");
    }

    private static void setSavedBuildDate(Context c, String date) {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(c);
